...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    const actions = [`~/.codex/scripts/codex-status ${job.id}`];
    if (job.status === "queued" || job.status === "running") {
      actions.push(`~/.codex/scripts/codex-cancel ${job.id}`);
    }
    lines.push(
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if ((job.status === "queued" || job.status === "running") && options.showCancelHint) {
    lines.push(`  Cancel: ~/.codex/scripts/codex-cancel ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && options.showResultHint) {
    lines.push(`  Result: ~/.codex/scripts/codex-result ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    lines.push("  Review changes: ~/.codex/scripts/codex-review --wait");
    lines.push("  Stricter review: ~/.codex/scripts/codex-adversarial-review --wait");
  }
...
export function renderCancelReport(job) {
  ...
  lines.push("- Check `~/.codex/scripts/codex-status` for the updated queue.");
