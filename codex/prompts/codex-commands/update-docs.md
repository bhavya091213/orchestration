...
## Step 0: Retrieve Local Knowledge

If the repository has `.obsidian/`, `docs/`, `wiki/`, `knowledge/`, `notes/`, `handbook/`, `playbooks/`, or markdown collections, retrieve only relevant context before editing:

```bash
~/.codex/scripts/codex-knowledge search "documentation conventions runbook contributing setup" --limit 8 --max-chars 12000
```

For delegated Markdown writing, use the `markdown_writer` agent role.
