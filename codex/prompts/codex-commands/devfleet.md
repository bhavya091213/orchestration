---
description: Orchestrate parallel Codex agents via Codex DevFleet — plan projects from natural language, dispatch agents in isolated worktrees, monitor progress, and read structured reports.
---

# DevFleet — Multi-Agent Orchestration

Orchestrate parallel Codex agents via Codex DevFleet. Each agent runs in an isolated git worktree with full tooling.

Requires the DevFleet MCP server: `codex mcp add devfleet --transport http http://localhost:18801/mcp`
...
- Missions that primarily write Markdown, Obsidian notes, prompt recipes, `AGENTS.md`, or `SKILL.md` should be dispatched with `model="gpt-5.4"` and a low-effort prompt. Include local knowledge retrieval instructions in the mission prompt instead of sending whole vaults or docs trees.
- For repositories with `.obsidian/`, `docs/`, `wiki/`, `knowledge/`, `notes/`, `handbook/`, or `playbooks/`, mission prompts should tell agents to run `~/.codex/scripts/codex-knowledge search "<mission-specific query>"` before acting and to re-query as needed.
