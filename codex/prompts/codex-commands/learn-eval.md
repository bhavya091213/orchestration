...
If this pattern depends on project docs, an Obsidian vault, or local markdown knowledge, run `~/.codex/scripts/codex-knowledge search "<task-specific query>"` before applying it and load only relevant chunks.
```
...
5. **Quality gate — Checklist + Holistic verdict**
   ...
   - [ ] If local knowledge sources exist, run `~/.codex/scripts/codex-knowledge search "<pattern keywords>"` and check for overlap or supporting context
...
This version replaces the previous 5-dimension numeric scoring rubric (Specificity, Actionability, Scope Fit, Non-redundancy, Coverage scored 1-5) with a checklist-based holistic verdict system. Modern frontier models (high-effort Codex profile 4.6+) have strong contextual judgment — forcing rich qualitative signals into numeric scores loses nuance and can produce misleading totals. The holistic approach lets the model weigh all factors naturally, producing more accurate save/drop decisions while the explicit checklist ensures no critical check is skipped.
...
- If a learned skill depends on a local knowledge base, include a bounded retrieval step and do not instruct future agents to load entire docs or vaults.
- If the extracted pattern is mainly Markdown writing, prefer the `markdown_writer` role (`gpt-5.4`, low effort) for future subagents.
