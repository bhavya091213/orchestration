...
This checklist is enforced by a `Codex workflow verification` hook that blocks completion until all steps are done. The hook intercepts `gmail send` / `conversations_add_message` and injects the checklist as a system reminder.
...
- **Hooks over prompts for reliability**: LLMs forget instructions ~20% of the time. `Codex workflow verification` hooks enforce checklists at the tool level — the LLM physically cannot skip them.
...
- [Codex](https://docs.anthropic.com/en/docs/codex-code)
