...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${ECC_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping Codex analysis on Windows due to known non-interactive hang risk (#295). Set ECC_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
...
PROMPT
  ...
  # Prevent observe.sh from recording this automated Codex session as observations.
  codex_cwd="${PROJECT_ROOT:-$PWD}"
  CODEX_ENTRYPOINT=automation \
    CLV2_SKIP_OBSERVE=1 \
    ECC_SKIP_OBSERVE=1 \
    ECC_HOOK_PROFILE=minimal \
    codex exec \
      --model "${CLV2_OBSERVER_MODEL:-gpt-5.4-mini}" \
      --skip-git-repo-check \
