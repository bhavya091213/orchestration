...
# Codex-compatible workflow adapters pass event data via stdin as JSON.
#
# v2.1: Project-scoped observations — detects current project context
#       and writes observations to project-specific directory.
#
# Native Codex lifecycle hooks are not assumed. Call this script from
# workflow scripts or git hooks when observation is desired.

set -e

# Hook phase from CLI argument: "pre" or "post"
HOOK_PHASE="${1:-post}"
...
# Read JSON from stdin (adapter event format)
INPUT_JSON=$(cat)
...
#   - the observer recording its own Codex analysis sessions (self-loop)
...
# Pass HOOK_PHASE via env var when the payload does not include hook type.
